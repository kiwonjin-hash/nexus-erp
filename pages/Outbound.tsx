...
const Outbound: React.FC = () => {
  ...
  const processTrackingSearch = async (tracking: string) => {
    ...
    console.log("[Outbound] 검색 시작:", { trackingNumbers, deliveryTypeFilter });
    ...
    try {
      ...
      for (const normalizedTracking of trackingNumbers) {
        ...
        console.log("[Outbound] 1차 검색 결과:", { normalizedTracking, found: !!docSnap, arrayCount: arraySnapshot.docs.length, legacyCount: legacySnapshot.docs.length });
        if (docSnap) break;
        ...
        console.log("[Outbound] 2차 fallback 결과:", { normalizedTracking, found: !!docSnap, arrayCount: arrayFallback.docs.length, legacyCount: legacyFallback.docs.length });
        if (docSnap) {
          console.log("[Outbound] fallback 문서 데이터:", docSnap.data());
          break;
        }
      }
